      ...
      <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
        ...
        <section className="border-t border-gray-800 py-8" aria-label="How it works">
          ...
          <p className="mt-3 leading-7 text-gray-400">
            ...
            Compression never guarantees a target size; you will see the actual result. MP4 exports have no audio and
            pad dimensions to even values of at least 16 × 16.
          </p>
          <nav aria-label="More GIF tools" className="mt-6 flex flex-wrap gap-5 text-[#9ff3ea]">
            {Object.entries(gifTools)
              .filter(([route]) => route !== tool)
              .map(([route, item]) => (
                <Link key={route} href={`/${route}`}>
                  {item.title}
                </Link>
              ))}
            <Link href="/screen-to-gif">Screen to GIF</Link>
          </nav>
        </section>
      </div>
      <script
